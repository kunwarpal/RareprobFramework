...
import com.rareprob.core_pulgin.payment.in_app_purchase.presentation.InAppBillingClientWrapper
import kotlinx.coroutines.flow.*
import java.io.IOException

class IapDataRepository(
    private val application: Application,
    private val billingClientWrapper: InAppBillingClientWrapper,
    ...
    fun getInAppProducts(): Flow<Resource<List<ProductListingData>>> = flow {
        ...
        try {
            ...
            dataList?.forEach { productDataItem ->
                var dbProduct = inAppPurchaseDao?.getInAppPurchaseById(
                    application.packageName,
                    productDataItem.productID ?: ""
                )
                var isPurchased = dbProduct?.isPurchased ?: false
